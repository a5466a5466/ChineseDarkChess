...
void MyAI::generateMove(char move[6])
{
	...
	for(int depth = 3; (double)(clock() - begin) / CLOCKS_PER_SEC < TIME_LIMIT; ++depth){
		...
		// run Nega-max
		t_tmp = Nega_max_alpha_beta(this->main_chessboard, &best_move_tmp, this->Color, alpha, beta, 0, depth);
...
//change to alpha beta pruning
double MyAI::Nega_max_alpha_beta(const ChessBoard chessboard, int* move, const int color, double alpha, double beta, const int depth, const int remain_depth){
	...
	if(isTimeUp() || // time is up
		...
		for(int i = 0; i < move_count; i++){ // move
			...
			double t = -Nega_max_alpha_beta(new_chessboard, &new_move, color^1, -beta, -m,  depth+1, remain_depth-1);
			if(t > m){
				m = t;
				*move = Moves[i];
			}
