...
void MyAI::generateMove(char move[6])
{
	...
	int* node_simulate_count = new int[move_count];
	double* Children_pure_Score = new double[move_count];
	double* node_total_win = new double[move_count];

	for(int i = 0; i < move_count; ++i){
		Children[i] = main_chessboard;
		MakeMove(&Children[i], Moves[i], 0); // 0: dummy
		Children_Scores[i] = 0; // reset
		node_simulate_count[i] = 0;// initialize
		Children_pure_Score[i] = 0;
		node_total_win[i] = 0;
	}

	// MCS_UCB 
	
	//do first round to let total_simulate_count unequal 0
	int total_simulate_count = 0;
	for(int i = 0; i < move_count; ++i){ 
		double inital_node_score = 0;
		//simulate 10 times each node
		for(int j=0; j < SIMULATE_COUNT_PER_CHILD; ++j){
			inital_node_score += Simulate(Children[i]);
		}
		node_total_win[i] = inital_node_score;
		node_simulate_count[i] += SIMULATE_COUNT_PER_CHILD;
		total_simulate_count += SIMULATE_COUNT_PER_CHILD;

		//UCB score
		Children_pure_Score[i] = (node_total_win[i]/node_simulate_count[i]);
		Children_Scores[i] = Children_pure_Score[i] + UCB_parameter*sqrt(log(total_simulate_count)/node_simulate_count[i]);
	}

	//print for verify
	//printf("After initialized: total_simulate_count==%d, node_simulate_count[1]==%d; [2]==%d **[1][2]needs to be 10", total_simulate_count, node_simulate_count[1], node_simulate_count[2]);
	/*
	for(int i=0; i< move_count;++i){
		printf("----------------------------------------\n");
		printf("node_total_win[%d] is %f\n", i, node_total_win[i]);
		printf("Children_pure_Score[%d] is %f\n", i, Children_pure_Score[i]);
		printf("Children_Scores[%d] is %f\n", i, Children_Scores[i]);
		printf("=========================================\n");
	}
	*/
	//after the first round simulation
	//each node_simulate_count[i] == 10
	//total_simulate_count ==100w

	//MCS_UCB
	while (!isTimeUp()){
		//find next node which is worth to simulate
		int max = 0;
		for(int i = 0; i < move_count; ++i){
			if(Children_Scores[max] < Children_Scores[i]){
				max = i;
			}
		}
		// printf("now we simulate the %d node and the score is: %f\n", max, Children_Scores[max]);

		//after choosing the best Children_score[max], we simulate next 10 times
		double simulate_node_score = 0;
		for(int j=0; j < SIMULATE_COUNT_PER_CHILD; ++j){
			simulate_node_score += Simulate(Children[max]);
		}
		node_simulate_count[max] += SIMULATE_COUNT_PER_CHILD;
		total_simulate_count += SIMULATE_COUNT_PER_CHILD;
		//printf("the total_simulate_count is:%d\n", total_simulate_count);
		//printf("the node_simulate_count[%d] is: %d\n", max, node_simulate_count[max]);


		//update all the node because this is UCB
		for(int i=0; i < move_count; ++i){
			if(i!=max){//other node
				Children_Scores[i] = Children_pure_Score[i] + UCB_parameter*sqrt(log(total_simulate_count)/node_simulate_count[i]);
			}else if(i == max){//simulated node
				node_total_win[max] += simulate_node_score;
				Children_pure_Score[max] = node_total_win[max]/node_simulate_count[max];
				Children_Scores[max] = Children_pure_Score[max] + UCB_parameter*sqrt(log(total_simulate_count)/node_simulate_count[max]);
			}else{
				printf("node update wrong\n");
			}
		}
	}
...
/*  past code MCT pure	
	while(!isTimeUp()){
		...
		for(int i = 0; i < move_count; ++i){
			double node_total_score = 0;
			for(int k = 0; k < SIMULATE_COUNT_PER_CHILD; ++k){
				node_total_score += Simulate(Children[i]);
			}
			total_simulate_count += SIMULATE_COUNT_PER_CHILD;
			Children_Scores[i] += node_total_score;
		}
...
*/
	...
	for(int i = 0; i < move_count; ++i){
		...
		fprintf(stderr, "%2d. Move: %s, Score: %+5lf, Sim_Count: %7d\n",
			i+1, tmp, Children_Scores[i], total_simulate_count);
		fflush(stderr);
	}
	...
	delete [] node_simulate_count;
	delete [] node_total_win;
}
